...
function DashboardSkeleton() {
  return (
    <div className="animate-pulse space-y-8">
      <div className="h-16 bg-[#1A1D27] border border-[#2A2D3A] rounded-2xl w-1/3" />
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map(i => <div key={i} className="h-40 bg-[#1A1D27] border border-[#2A2D3A] rounded-2xl" />)}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 h-[300px] bg-[#1A1D27] border border-[#2A2D3A] rounded-2xl" />
        <div className="lg:col-span-1 h-[300px] bg-[#1A1D27] border border-[#2A2D3A] rounded-2xl" />
      </div>
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  // getPlatformStats() returns res.data = { success, data: { stats, recentRegistrations, ... } }
  // so we must unwrap through .data first
  const stats = data?.data?.stats ?? {};
  const recentRegistrations = data?.data?.recentRegistrations ?? [];
  const pendingVendors = data?.data?.pendingVendors ?? [];
  const monthlyRegistrations = data?.data?.monthlyRegistrations ?? [];
  const categoryBookings = data?.data?.categoryBookings ?? [];
  const chartData = monthlyRegistrations;
    ...
    <PageTransition className="min-h-screen bg-[#0F1117] p-6 lg:p-8 font-sans">
      <div className="max-w-[1400px] mx-auto space-y-8">
          ...
          <div className="group bg-[#1A1D27] border border-[#2A2D3A] hover:border-[var(--color-gold)]/40 rounded-2xl p-6 transition-all duration-300 hover:shadow-[0_0_20px_rgba(201,162,77,0.1)] flex flex-col justify-between h-40 relative overflow-hidden">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-2xl font-extrabold text-white">{(stats.totalUsers ?? 0).toLocaleString()}</h3>
              </div>
          ...
          <div className="group bg-[#1A1D27] border border-[#2A2D3A] hover:border-[var(--color-gold)]/40 rounded-2xl p-6 transition-all duration-300 hover:shadow-[0_0_20px_rgba(201,162,77,0.1)] flex flex-col justify-between h-40 relative overflow-hidden">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-2xl font-extrabold text-white">{(stats.approvedVendors ?? 0).toLocaleString()}</h3>
          ...
          <div className="group bg-[#1A1D27] border border-[#2A2D3A] hover:border-[var(--color-gold)]/40 rounded-2xl p-6 transition-all duration-300 hover:shadow-[0_0_20px_rgba(201,162,77,0.1)] flex flex-col justify-between h-40 relative overflow-hidden">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-2xl font-extrabold text-white">{(stats.totalBookings ?? 0).toLocaleString()}</h3>
              </div>
          ...
          <div className="group bg-[#1A1D27] border border-[#2A2D3A] hover:border-[var(--color-gold)]/40 rounded-2xl p-6 transition-all duration-300 hover:shadow-[0_0_20px_rgba(201,162,77,0.1)] flex flex-col justify-between h-40 relative overflow-hidden">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-2xl font-extrabold text-white">{(stats.grossRevenue ?? 0).toLocaleString()} <span className="text-sm font-medium text-[#8B8FA8]">JOD</span></h3>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Right: Revenue / Activity (placeholder for Category Bookings) */}
          <div className="lg:col-span-1 bg-[#1A1D27] border border-[#2A2D3A] rounded-2xl p-6">
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer minWidth={0} minHeight={0} width="100%" height="100%">
                <BarChart data={[]} layout="vertical" margin={{ top: 0, right: 20, bottom: 0, left: 10 }}>
                  ...
                  />
                  <Bar dataKey="value" name="Bookings" fill="#6366f1" radius={[0, 4, 4, 0]} barSize={20} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <table className="w-full text-left border-collapse">
                <tbody>
                  {recentRegistrations.length === 0 ? (
                    <tr><td colSpan="3" className="p-4 text-[#8B8FA8] text-center text-sm">No recent registrations.</td></tr>
                  ) : (
                    recentRegistrations.map((user, idx) => (
                      <tr key={user.user_id} className={`border-b border-[#2A2D3A] hover:bg-[#22253A] transition-colors ${idx === recentRegistrations.length - 1 ? 'border-b-0' : ''}`}>
                        ...
                        <td className="p-4 text-xs font-medium text-[#8B8FA8]">{new Date(user.date).toLocaleDateString()}</td>
                        <td className="p-4 pr-6 text-right">
                          {user.is_active ? (
                            <div className="inline-flex items-center justify-center w-2 h-2 rounded-full bg-emerald-500" title="Active" />
                          ) : (
                            <div className="inline-flex items-center justify-center w-2 h-2 rounded-full bg-amber-500" title="Pending/Banned" />
                          )}
                        </td>
                      </tr>
                    ))
          ...
          <div className="bg-[#1A1D27] border border-[#2A2D3A] rounded-2xl flex flex-col relative overflow-hidden">
            ...
            <div className="p-6 border-b border-[#2A2D3A] flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold text-white">{stats.pendingVendors ?? 0} Vendors waiting for approval</h2>
            </div>
            <div className="flex-1 p-4 space-y-3">
              {pendingVendors.length > 0 ? (
                pendingVendors.map((vendor) => (
                  <div key={vendor.vendor_id} className="bg-[#0F1117] border border-[#2A2D3A] rounded-xl p-4 flex items-center justify-between hover:border-[var(--color-gold)]/40 transition-colors">
                    <div>
                      <h3 className="text-sm font-bold text-white">{vendor.company}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-[10px] text-[#8B8FA8] bg-[#1A1D27] px-2 py-0.5 rounded">{vendor.category || 'Vendor'}</span>
                        <span className="text-[10px] text-[#8B8FA8] flex items-center gap-1">
                          <Clock size={10} /> {new Date(vendor.submitted).toLocaleDateString()}
                        </span>
                      </div>
                    </div>
